...
const PROVIDERS = [
  ...
  // Ben geeft 403 op elke automatische browser, lokaal en op de runner. Acht
  // pogingen van veertig seconden leverden niets op behalve een run van bijna
  // acht minuten. Overslaan tot er een bron is; status blijft 'onbekend'.
  { id: 'ben', naam: 'Ben', overslaan: true, urls: [
  ...
  try {
    ...
    for (const sel of CONSENT) {
      try {
        ...
        if (await el.count() && await el.isVisible({ timeout: 400 })) { await el.click({ timeout: 1500 }); break }
      } catch {}
    }
...
for (const p of PROVIDERS) {
  ...
  if (p.overslaan) {
    nieuw.providers[p.id] = { naam: p.naam, url: p.urls[0], status: 'onbekend',
      laatsteFout: 'overgeslagen: blokkeert automatische toegang' }
    console.log(`- ${p.naam.padEnd(16)} overgeslagen (blokkeert automatische toegang)`)
    continue
  }
  try {
